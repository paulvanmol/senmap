...
%shpimprt(shapefilepath=&path/ShapefileSNG/&level/&level.s_SN.shp,
			ID=CODE_Reg,
...
drop=	  pop_hom pop_fem pop_total area perimetre pole) 
&maplib..senmap_&level._attr (promote=yes replace=yes 
keep=code_reg ncode_reg nom pop_hom pop_fem pop_total area perimetre pole);
set casuser.senmap_&level (rename=(code_reg=ncode_reg));
length Code_reg $12;
code_reg=cats('SN',ncode_reg); 
*drop 
...
proc sort data=MAPSCSTM.SENMAP_REGION_ATTR 
		out=MAPSCSTM.SENMAP_REGION_ATTROK nodupkey equals;
	by ncode_reg Code_reg;
run;
cas mysession terminate;