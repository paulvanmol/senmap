%let shapefile=Carte_Commune_SN_VF_Zone_DRV-polygon.shp; 
...
%shpcntnt(shapefilepath=&path/ShapefileSNG/&level/&shapefile)

%shpimprt(shapefilepath=&path/ShapefileSNG/&level/&shapefile,
			ID=&id,
...
keep=&id cod_reg cod_dept cod_ccrca cod_cav segment x y __seq__ density) 
&maplib..senmap_&level._attr (promote=yes replace=yes 
drop=segment x y __seq__ density);
